...
.bc_collapse_identical <- function(aln, locus) {
  ...
  representante <- unname(first_sid[key])
  map <- data.frame(locus = rep(locus, length(aln)), species = h$species, sid = h$sid,
                    representante = representante, colapsada = h$sid != representante,
                    stringsAsFactors = FALSE)
  list(aln = aln[!map$colapsada], map = map)
}
...
#' whenever those directories exist. The loci are those marked `entra` in the screening table.
#'
...
finalize_barcoding_library <- function(assembly_dir = file.path("11_barcoding", "1_assembly"),
  ...
  loci <- screening$locus[screening$entra %in% TRUE]
  ...
  for (i in seq_along(loci)) {
    ...
    if (file.exists(log_file)) {
      ...
      if (nrow(lg) > 0) {
        ...
        excluded[[l]] <- data.frame(
          ...
          descripcion = if (is.null(meta)) NA_character_ else meta$Description_gb[match(h$sid, meta$sid)],
          motivo = "no_match_either_direction",
          share_forward = lg$ShareForward, share_reverse = lg$ShareReverse,
          en_alineamiento_curado = lg$Seq %in% names(aln),
          stringsAsFactors = FALSE
        )
      }
    } else {
    ...
    pasa <- ctx$especies_con_replica >= min_species_with_replica
    rows[[l]] <- data.frame(ctx,
                            especies_con_replica_antes_colapso = n_rep_before,
                            accesiones_excluidas_no_homologas = n_excluded,
                            accesiones_colapsadas = sum(col$map$colapsada),
                            pasa_umbral = pasa, entra = pasa,
                            stringsAsFactors = FALSE)
    if (pasa) {
    ...
    message(sprintf("Locus '%s': %d non-homologous removed, %d identical collapsed, %d species with replica (%s).",
                    l, n_excluded, sum(col$map$colapsada), ctx$especies_con_replica,
                    if (pasa) "enters" else "does not enter"))
  }

  summary_tab <- do.call(rbind, rows)
  rownames(summary_tab) <- NULL
  excluded_tab <- if (length(excluded) > 0) do.call(rbind, excluded) else
    data.frame(locus = character(0), sid = character(0), species = character(0), descripcion = character(0),
               motivo = character(0), share_forward = numeric(0), share_reverse = numeric(0),
               en_alineamiento_curado = logical(0), stringsAsFactors = FALSE)
  ...
  funnel_tab <- do.call(rbind, lapply(all_loci, function(l) {
    ...
    data.frame(
      locus = l,
      accesiones_ensamblado = sum(registry$locus == l),
      accesiones_curadas = as.integer(n_cur),
      entra_cribado = l %in% loci,
      accesiones_excluidas_no_homologas = if (done) s$accesiones_excluidas_no_homologas else NA_integer_,
      accesiones_colapsadas = if (done) s$accesiones_colapsadas else NA_integer_,
      accesiones_tras_colapso = if (done) unname(n_after[l]) else NA_integer_,
      especies_con_replica = if (done) s$especies_con_replica else NA_integer_,
      entra = if (done) s$entra else FALSE,
      stringsAsFactors = FALSE
    )
  }))
