...
#' not carried in memory, any stage of the pipeline can be executed in a fresh R session
...
#' stage clears those variables, and the stage fails on a missing object even when the file exists. Hard-coding the filenames instead removes the dependency on session state but reintroduces
...
#' each stage independently runnable. Two entries are resolved by inspecting the run directory,
#' because they record facts about the run, not naming choices:
#'
#' \itemize{
...
#'
#' The trailing underscore in the default pattern is required. Terminals use
