...
#'
#' Compares the focal tree produced by `PhyloCactus` with published phylogenies (e.g., Amaral *et al.* 2022,
#' Thompson *et al.* 2024, Zuntini *et al.* 2024, de Vos *et al.* 2025). Pairwise distances are computed with the
#' Robinson-Foulds distance (Robinson & Foulds, 1981) and the Different Phylogenetic Information distance
#' (`TreeDist`; Smith, 2020), both on unrooted trees, and projected into tree space by multidimensional scaling (MDS).
#'
...
#' @param tree_labels Named character vector or `NULL`. Legend labels for the trees in the tree-space
#'   figure, named by the entries of `trees_mapping_list`. Labels of the form `"Author et al. (year)"`
#'   are typeset with *et al.* in italics. Defaults to `NULL`, which labels the reference trees
#'   distributed with the package as `"Zuntini et al. (2024)"`, `"Thompson et al. (2024)"`,
#'   `"Amaral et al. (2022)"` and `"de Vos et al. (2025)"`, and the focal tree as `"This study"`.
...
#'
#'   This differs from the rooting used elsewhere in `PhyloCactus`. The comparison is restricted to
#'   **Cactaceae**, so the family filter removes the outgroup terminals used by the dating pipeline.
#'   *Leuenbergeria* is sister to the rest of the family in the focal tree. The distances and the
#'   tree-space projection are computed on unrooted trees and do not depend on this choice; the
#'   rooting affects only the exported trees. All terminals of the genus are used, not one, for the reasons
...
#'
#' Smith, M. R. (2020). Information theoretic generalized Robinson-Foulds metrics for comparing phylogenetic trees.
...
#' }
#' @export
validate_phylogenies <- function(trees_mapping_list, checklist_csv, constraints_map, out_dir, min_tips = 4L, pruning_strategy = c("common_set", "pairwise"), rooting_genus = "Leuenbergeria", tree_labels = NULL) {
  ...
  # Shapes as well as colours, so that the five trees remain distinguishable in greyscale print and
  # when two points overlap.
  tree_shapes <- c(FocalTree = 16, Zuntini = 17, Thompson = 15, Amaral = 18, deVos = 4)

  reference_labels <- c(
    FocalTree = "This study",
    Zuntini   = "Zuntini et al. (2024)",
    Thompson  = "Thompson et al. (2024)",
    Amaral    = "Amaral et al. (2022)",
    deVos     = "de Vos et al. (2025)"
  )
  if (!is.null(tree_labels)) {
    if (!is.character(tree_labels) || is.null(names(tree_labels)) || any(!nzchar(names(tree_labels)))) {
      stop("`tree_labels` must be a named character vector.", call. = FALSE)
    }
    reference_labels[names(tree_labels)] <- tree_labels
  }
  legend_labels <- .format_reference_labels(reference_labels[names(tree_palette)])
  ...
  plot_tree_space_panel <- function(coords_tbl, analysis_id_target, title_label = NULL,
    ...
    # Trees are identified by colour and shape in a single legend, labelled by source. Point labels
    # inside the panels are not drawn.
    ggplot2::ggplot(df, ggplot2::aes(x = Dimension_1, y = Dimension_2, color = tree, shape = tree)) +
...
#' Typeset source labels for a figure legend
#'
#' Turns labels of the form `"Author et al. (year)"` into plotmath expressions with *et al.* in
#' italics, and leaves any other label as plain text.
#'
#' @param x Character vector of labels.
#' @return An expression vector of the same length, for the `labels` argument of a ggplot2 scale.
#' @keywords internal
#' @noRd
.format_reference_labels <- function(x) {
  quote_text <- function(s) paste0('"', gsub('"', '\\"', s, fixed = TRUE), '"')
  out <- vapply(unname(x), function(s) {
    m <- regmatches(s, regexec("^(.*) et al\\. \\(([0-9]{4}[a-z]?)\\)$", s))[[1]]
    if (length(m) == 3L) {
      paste0(quote_text(m[2]), '~italic("et al.")~', quote_text(paste0("(", m[3], ")")))
    } else {
      quote_text(s)
    }
  }, character(1))
