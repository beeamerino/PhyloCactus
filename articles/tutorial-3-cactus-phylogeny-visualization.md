...
In **Module 12**, the final phylogenetic products and molecular datasets
are visualized and synthesized for downstream comparative analyses. The
sequence dataset and concatenated supermatrix are evaluated for locus
coverage and informativeness, while the maximum-likelihood topology and
time-calibrated chronogram are displayed with branch support values
(FBP) and divergence time uncertainty intervals. Finally, taxonomic
reconciliation and macroevolutionary patterns across IUCN conservation
categories, endemism, and ecological threats are synthesized into
...
Taxonomic reconciliation is conducted against the dynamic Cactaceae
species checklist published by the Caryophyllales.org initiative
(Korotkova *et al*. 2021), comprising 1,965 accepted species. When
combined with 38 outgroup species from Anacampserotaceae, Portulacaceae,
and Talinaceae, the total taxonomic evaluation universe encompasses
2,003 taxa. Across the pipeline, 1,060 species possess sequence data in
the multilocus supermatrix (1,022 ingroup and 38 outgroup), while the
inferred maximum-likelihood tree and dated chronogram resolve 1,023
terminal tips (985 ingroup and 38 outgroup). Exactly 37 ingroup species
were collapsed by `RAxML-NG` due to identical sequence profiles across
their sampled loci. Crucially, the 943 accepted Cactaceae species
lacking sequence data represent critical gaps for biodiversity
inventory, among which 134 are categorized as threatened (CR, EN, VU) by
the IUCN Red List.
...
(Korotkova *et al*. 2021) before retrieving conservation information.
Only validated species-level names are queried against the IUCN API,
reducing false matches and ensuring that conservation metadata are
associated with the appropriate evolutionary units.
...
Following the completion of maximum-likelihood inference, divergence
time estimation, and biological metadata integration, this module
presents the complete analytical synthesis of `PhyloCactus`.

To establish a clear scientific progression, the visualizations are
structured into five consecutive analytical parts: 1. **Molecular
Dataset and Alignment Properties (Figures 1 to 3)**: Taxonomic marker
representation, locus completeness per species, and locus occupancy
heatmaps. 2. **Supermatrix Concatenation Structure (Figures 4 to 5)**:
Partition architecture, boundary coordinates, and parsimony-informative
site contributions. 3. **Inferred Phylogeny and Divergence Timescale
(Figures 6 to 9)**: Maximum-likelihood topology with Felsenstein’s
Bootstrap Proportions (FBP), high-resolution uncollapsed A0 poster,
time-calibrated chronogram with 95% HPD intervals, and extended
chronogram poster. 4. **Taxonomic Reconciliation and Species Cadastre
(Figure 10)**: Global species accounting comparing the taxonomic
checklist, supermatrix sampling, collapsed duplicates, and phylogenetic
placement. 5. **IUCN Conservation and Macroevolutionary Syntheses
(Figures 11 to 17)**: Extinction risk distributions, molecular sampling
completeness across threat categories, conservation prioritization of
unsampled gaps, national endemism rankings, threat profiles, and
ecological habitat associations.
...
marker retained in the final dataset by quantifying the number of
species with available sequence data across locus partitions.
...
completeness across taxa resolved in the maximum-likelihood tree,
indicating the number of retained loci per species following sequence
...
This heatmap provides an overview of locus occupancy across all species
resolved in the phylogeny, illustrating patterns of data presence and
missingness across individual marker partitions.
...
This figure depicts the linear architecture of the concatenated
supermatrix, displaying the start and end nucleotide coordinates for
each locus partition.
...
We evaluate the phylogenetic utility of each partition by plotting the
number of parsimony-informative sites against total alignment length.
...
This plot renders the inferred maximum-likelihood topology annotated
with Felsenstein’s Bootstrap Proportions (FBP) mapped directly onto
internal nodes, alongside major clade demarcation layers.
...
To examine all 1,023 terminal tips without node compression, this
visualization formats the complete topology as an A0-dimension poster,
displaying individual species binomials and node-level bootstrap
support.
...
For interactive exploration in external phylogenetic software (such as
FigTree), the annotated `treedata` object can be exported into
Nexus/BEAST format:
...
> **Note on Extended Tree Poster & Direct Downloads:** Due to its large
> format dimensions (A0 poster: 36 × 48 inches / 91.4 × 121.9 cm) and
> 1,023 terminal tips, this extended tree is not rendered inline within
> this document to maintain browser responsiveness and prevent visual
> overcrowding.
>
> When executing this workflow locally, the full vector PDF and
> annotated BEAST tree file are generated directly in your workspace: -
> Local PDF: `9_Visualization/figures/Figure_7_ML_Tree_Extended.pdf` -
> Local Tree: `9_Visualization/figures/annotated_ml_tree.tree`
>
> Once the online repository is published on GitHub, these
> high-resolution assets can be downloaded directly from the repository
> [`extended_trees/`](https://github.com/beeamerino/PhyloCactus/tree/main/extended_trees)
> directory: - Direct PDF Download:
> [`Figure_7_ML_Tree_Extended.pdf`](https://raw.githubusercontent.com/beeamerino/PhyloCactus/main/extended_trees/Figure_7_ML_Tree_Extended.pdf) -
> Direct Tree Download:
> [`annotated_ml_tree.tree`](https://raw.githubusercontent.com/beeamerino/PhyloCactus/main/extended_trees/annotated_ml_tree.tree)

#### Figure 8: Time-Calibrated Chronogram with HPD Intervals

This chronogram displays absolute divergence times estimated via
penalized likelihood (`treePL`) and summarized across temporal bootstrap
replicates, with node age uncertainty intervals (95% HPD) and a
calibrated geological timescale axis.
...
This A0-dimension poster presents the uncollapsed temporal chronogram
with all 1,023 tip names, divergence time confidence intervals, and
...
> **Note on Extended Chronogram Poster & Direct Downloads:** Due to its
> large format dimensions (A0 poster: 36 × 48 inches / 91.4 × 121.9 cm),
> 1,023 terminal tips, and node age uncertainty intervals (95% HPD),
> this extended chronogram is not rendered inline within this document
> to maintain browser responsiveness.
>
> When executing this workflow locally, the full vector PDF and
> annotated BEAST chronogram file are generated directly in your
> workspace: - Local PDF:
> `9_Visualization/figures/Figure_9_Chronogram_Extended.pdf` - Local
> Tree: `9_Visualization/figures/annotated_chronogram.tree`
>
> Once the online repository is published on GitHub, these
> high-resolution assets can be downloaded directly from the repository
> [`extended_trees/`](https://github.com/beeamerino/PhyloCactus/tree/main/extended_trees)
> directory: - Direct PDF Download:
> [`Figure_9_Chronogram_Extended.pdf`](https://raw.githubusercontent.com/beeamerino/PhyloCactus/main/extended_trees/Figure_9_Chronogram_Extended.pdf) -
> Direct Tree Download:
> [`annotated_chronogram.tree`](https://raw.githubusercontent.com/beeamerino/PhyloCactus/main/extended_trees/annotated_chronogram.tree)

#### Figure 10: Species Composition (Phylogeny vs. Checklist by Pipeline Status)

This figure summarizes the methodological composition of the taxonomic
universe across pipeline stages, distinguishing taxa that were sampled
and dated in the tree (985), sequenced identical duplicates collapsed by
`RAxML-NG` (37), unsampled checklist gaps (943), and outgroup reference
species (38).
...
This bar chart summarizes the distribution of IUCN Red List conservation
categories among the ingroup species resolved in the final phylogenetic
reconstruction.
...
This plot compares molecular marker completeness against IUCN threat
categories, faceted by pipeline status (`sampled_and_dated` vs
`sequenced_collapsed_duplicate`), to test whether extinction risk
correlates with missing molecular data or sequence collapse during
...
This figure visualizes the taxonomic distribution of threatened species
(CR, EN, VU) among the 943 checklist gaps that currently lack molecular
sequences in public repositories. By highlighting genera with the
highest concentrations of unsequenced threatened taxa (led by *Parodia*,
*Mammillaria*, and *Melocactus*), this analysis establishes explicit
priorities for targeted field collecting and sequencing.
...
This figure summarizes national cactus endemism represented in the
phylogenetic tree, ranking countries by the number of endemic species
resolved in the molecular framework.
...
This stacked bar chart visualizes the conservation status of national
endemics across the top cactus-diverse countries, breaking down endemic
richness by IUCN threat category.
...
This bar chart illustrates the most frequently documented environmental
and anthropogenic threats impacting cactus species in the ingroup
phylogeny, based on IUCN Red List threat classifications.
...
This bar chart displays the primary habitat associations recorded for
species in the ingroup phylogeny, identifying major ecological biomes
supporting cactus diversification.
...
- Korotkova, N., Aquino, D., Arias, S., Eggli, U., Franck, A.,
  Gómez-Hinostrosa, C., Guerrero, P. C., Hernández, H. M., Kohlbecker,
  A., Köhler, M., Luther, K., Majure, L. C., Müller, A., Metzing, D.,
  Nyffeler, R., Sánchez, D., Schlumpberger, B. O., &
  Berendsohn, W. G. 2021. Cactaceae at Caryophyllales.org - a dynamic
  online species checklist. *PhytoKeys*, 186, 7-134.
  <https://doi.org/10.3897/phytokeys.186.73233>
